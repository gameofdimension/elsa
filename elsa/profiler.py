...
PROFILER_DIR = os.getenv("ELSA_TORCH_PROFILER_DIR", "/tmp/elsa_profiles")
...
class ProfilerContext:
    ...
    def __enter__(self):
        ...
        torch_activities = [activity_map[a]
                            for a in self.activities if a in activity_map]
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if self.profiler is not None:
            ...
            logger.info(
                f"Profiling completed. Trace saved to: {self.trace_path}")
...
def get_profiler_output_dir() -> str:
    return os.environ.get("ELSA_TORCH_PROFILER_DIR", PROFILER_DIR)


def set_profiler_output_dir(path: str):
    os.environ["ELSA_TORCH_PROFILER_DIR"] = path
